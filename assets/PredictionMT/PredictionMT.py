...
class CreateWindow(wx.Frame):
    ...
    def __init__(self, comm_obj, msg):
        ...
        # train button
        self.train_btn = wx.Button(self.panel, label="Train", pos=(135, 30), size=(100, 25))
        self.train_btn.Bind(wx.EVT_BUTTON, self.start_training)

        # revert button to hide/unhide rollback and version list
        self.revert_btn = wx.Button(self.panel, label="Revert", pos=(245, 30), size=(100, 25))
        self.revert_btn.Bind(wx.EVT_BUTTON, self.show_rollback)

        # rollback button to rollback to a previous version
        self.rollback_btn = wx.Button(self.panel, label="Rollback", pos=(355, 30), size=(100, 25))
        ...
        # log message box
        self.log = wx.TextCtrl(self.panel, pos=(20, 70), size=(450, 110) , style=wx.TE_MULTILINE|wx.TE_READONLY)
    ...
    def start_training(self, event):
          ...
          self.comm_obj.percentageIncri(self.msg,10,"Veryfing config data...")
        ...
        ##                'tensorflow_hub/tools/make_image_classifier')
          ...
          if not FLAGS.image_dir:
            ...
            return -1
          self.comm_obj.percentageIncri(self.msg,15,"Prepare training directories...")
          # Prepare necessary directories that can be used during training
          prepare_file_system()

          self.comm_obj.percentageIncri(self.msg,20,"Creating list of all images...")
          ...
          self.comm_obj.percentageIncri(self.msg,25,"Check for distortion...")
          # See if the command-line flags mean we're applying any distortions.
          do_distort_images = should_distort_images(
              FLAGS.flip_left_right, FLAGS.random_crop, FLAGS.random_scale,
              FLAGS.random_brightness)

          self.comm_obj.percentageIncri(self.msg,40,"Setting up pre-trained graph...")
          # Set up the pre-trained graph.
          module_spec = hub.load_module_spec(FLAGS.tfhub_module)
          graph, bottleneck_tensor, resized_image_tensor, wants_quantization = (
              create_module_graph(module_spec))

          self.comm_obj.percentageIncri(self.msg,45,"Adding new layer to train...")
          ...
          self.comm_obj.percentageIncri(self.msg,50,"Training...")
          with tf.Session(graph=graph) as sess:
            ...
            self.comm_obj.percentageIncri(self.msg,55,"Setting up image decoding sub-graph...")
            ...
            self.comm_obj.percentageIncri(self.msg,60,"Evaluating accuracy...")
            ...
            self.comm_obj.percentageIncri(self.msg,65,"Training datasets...")
            ...
            self.comm_obj.percentageIncri(self.msg,70,"Saving Checkpoint...")
            # After training is complete, force one last save of the train checkpoint.
            train_saver.save(sess, FLAGS.checkpoint_path)

            self.comm_obj.percentageIncri(self.msg,75,"Evaluating images...")
            ...
            self.comm_obj.percentageIncri(self.msg,80,"Saving trained data...")
            ...
            self.comm_obj.percentageIncri(self.msg,85,"Saving Changes in Config Files...")
            self.upgrade_version()
            self.reset_dropdown()
            self.comm_obj.percentageIncri(self.msg,90,"Removing Temporary Files...")
            self.remove_temporary_files()
            self.comm_obj.percentageIncri(self.msg,95,"Successfully trained!")
    ...
    def backup_current_model(self):
        try:
            ...
            if os.path.exists(self.active_model):
                ...
                active_model = self.active_model[:self.active_model.index('.pb')]
                os.rename(self.active_model, active_model+'_'+self.current_version+'.pb')
                archive_command = r'{loc_7z} a {store_loc} {source_file}'.format(loc_7z=LOC_7z, store_loc = self.rollback_dir, source_file = active_model+'_'+self.current_version+'.pb')
                subprocess.call(archive_command, shell=True)
                #log.debug("Model Backed Up!")
                os.rename(active_model+'_'+self.current_version+'.pb', self.active_model)
    ...
    def upgrade_version(self):
        try:
            ...
            with open(config_path, "w") as outfile:
                ...
                # current version of Prediction MT is updated
    ...
    def show_rollback(self, event):
        if self.rollback_btn.IsShown():
            ...
            self.train_btn.SetPosition((25, 30))
    ...
    def delete_backups(self, index):
        if not os.path.exists(self.rollback_dir):
            ...
            return
        try:
            versions = "retrained_graph_"+"retrained_graph_".join(self.all_versions[index+1:])
    ...
    def rollback(self, event):
        ...
        if self.backup_current_model():
            ...
            try:
                extract_model = r'{loc_7z} x {zipfile} -o{outputdir} {file} -r -y'.format(loc_7z=LOC_7z, zipfile=self.rollback_dir, outputdir=os.path.dirname(self.active_model), file=str("retrained_graph_"+rollback_ver+'.pb'))
                subprocess.call(extract_model, shell=True)
                self.comm_obj.percentageIncri(self.msg,70,"Rolling Back...")
                if os.path.exists(self.active_model) and os.path.exists(active_model+'_'+rollback_ver+'.pb'):
