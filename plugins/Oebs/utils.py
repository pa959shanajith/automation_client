...
##        time.sleep(0.120)
    ...
    def get_foreground_window(self, *args):
        ...
        err_msg=None
        try:
            aut_handle = win32gui.FindWindow(None,args[3][0])
            if aut_handle> 0:
                foreground=win32gui.GetForegroundWindow()
                pid=win32process.GetWindowThreadProcessId(aut_handle)
                foreground_pid=win32process.GetWindowThreadProcessId(foreground)
                if pid!=foreground_pid:
                    ...
                    if i[0] <= -32000:
                        ...
                        win32process.AttachThreadInput(aut_thread, fg_thread, True)
                        win32gui.BringWindowToTop(aut_handle)
                        win32gui.ShowWindow(aut_handle,4)
                        #win32gui.SetForegroundWindow(aut_handle)
                        status =TEST_RESULT_PASS
                        methodoutput =TEST_RESULT_TRUE
                        output = "Window with {} window name brought to foreground".format(args[3][0])
                    else:
                        win32gui.BringWindowToTop(aut_handle)
