...
TOGGLE_CHECK_THROUGH_PROPERTY = """
                                debugger;
                                if (arguments[0].ariaChecked=='true' || arguments[0].checked) 
                                    return "ON";
                                return "OFF" 
                                """

TOGGLE_CHECK_THROUGH_POSITION = """ 
                                    debugger; 
                                    var blob = arguments[0];
                                    var slider = blob.parentElement;
                                    while(slider.getBoundingClientRect().height*slider.getBoundingClientRect().width==0)
                                        slider = slider.parentElement;
                                    if (slider.getBoundingClientRect().x==blob.getBoundingClientRect().x)
                                        return "OFF";
                                    return "ON";
                                """
...
FIND_CUSTOM_ELEMENT = """debugger;
                    var ref_ele = arguments[0], type = arguments[1], index = arguments[2],text = arguments[3], elems = ref_ele.getElementsByTagName(type);                                                                                                                                                                    
...
BROWSER_NAME_MAP = {'chrome': 'Chrome', 'firefox': 'Firefox', 'internet explorer': 'Internet Explorer', 'MicrosoftEdge': 'Edge Chromium', 'msedge': 'Edge Chromium', 'Safari': 'Safari', 'chrome-headless-shell': 'Chrome', 'edge-headless-shell': 'Edge Chromium'}
