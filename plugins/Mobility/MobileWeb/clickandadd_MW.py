...
import logging
log = logging.getLogger('clickandadd_MW.py')
currenthandle = ''
status = domconstants_MW.STATUS_FAIL


class clickandadd_MW():
    def startclickandadd_MW(self):
        try:
            global currenthandle
            log.info('Inside startclickandadd_MW method .....')
...
##            win32gui.ShowWindow(actwindow, win32con.SW_MAXIMIZE)
            ...
            time.sleep(6)
            driver.execute_script(javascript_hasfocus)
            if SYSTEM_OS != "Darwin":
            ...
            def callback_clicknadd1(myipath):
                ...
                for iframes in (list(range(len(driver.find_elements_by_tag_name(domconstants_MW.IFRAME))))):
                    ...
                    if(switchtoframe_clicknadd1(path)):
                        ...
                        for frames in (list(range(len(driver.find_elements_by_tag_name(domconstants_MW.IFRAME))))):
            ...
            def callback_clicknadd2(myipath):
                path = myipath
                for iframes in (list(range(len(driver.find_elements_by_tag_name(domconstants_MW.FRAME))))):
                    #custom switchtoframe:
            ...
            callback_clicknadd1('')
            log.info('clickandadd_MW scrape operation on iframe/frame pages is completed')

            if SYSTEM_OS!= "Darwin":
                driver.switch_to.window(currenthandle)
            callback_clicknadd2('')
            log.info('clickandadd_MW scrape operation on frame/iframe pages is completed')
            if SYSTEM_OS!= "Darwin":
                driver.switch_to.window(currenthandle)
...
##        vie = {}
        ...
        try:
            ...
            log.info('stopclickandadd_MW operation on iframe/frame pages is completed')
            if SYSTEM_OS!= "Darwin":
                driver.switch_to.window(currenthandle)
            callback_stopclicknadd2('', tempne_stopclicknadd)
            log.info('stopclickandadd_MW operation on frame/iframe pages is completed')
            if SYSTEM_OS!= "Darwin":
                driver.switch_to.window(currenthandle)
##            global vie
...
##            data.append(screenshot)
            ...
            if driver.current_url == domconstants_MW.BLANK_PAGE:
                log.info('url is blank so cannot perform clickandadd_MW operation')
                status = domconstants_MW.STATUS_FAIL
