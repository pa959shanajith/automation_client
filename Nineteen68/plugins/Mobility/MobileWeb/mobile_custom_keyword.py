...
class CustomKeyword:
    ...
    def switch_to_parent(self):
        ...
        curr_window_handle=mobile_browser_keywords.driver_obj.current_window_handle
        mobile_browser_keywords.driver_obj.switch_to.window(curr_window_handle)
    ...
    def find_object(self,array_index, ele_type, visible_text, url, ele_index,local_index):
        return_list=mobile_browser_keywords.driver_obj.execute_script(CUSTOM_JS,'', array_index, ele_type, visible_text, ele_index,local_index)
        if return_list[0] == 'null':
            ...
            mobile_browser_keywords.driver_obj.switch_to.frame(iframe_ele)

            iframe_list = mobile_browser_keywords.driver_obj.execute_script(CUSTOM_IFRAME_JS,'', 0,ele_type, visible_text, ele_index,return_list[2])
        ...
        if not(ele_type is None or ele_type=='' or visible_text is None or ele_index is None):
            ...
            try:
                ...
                if ele_index<0:
                    ...
                    array_index=mobile_browser_keywords.driver_obj.execute_script(FIND_INDEX_JS,reference_ele)
    ...
    def get_count(self,counter,ele_type,index):
        result=mobile_browser_keywords.driver_obj.execute_script(GET_OBJECT_COUNT_JS,counter,ele_type,index,self.list_flag)
        counter=result[0]
        index=result[2]
        index=index+1
        if len(result)>3:
    ...
    def get_count_iframe(self,iframe_ele,counter,ele_type):
        ...
        mobile_browser_keywords.driver_obj.switch_to.parent_frame()
        return counter
    ...
    def getElementXPath(self,webelement):
        try:
            return mobile_browser_keywords.driver_obj.execute_script(GET_XPATH_SCRIPT,webelement)
        except Exception as e:
            log.error(e)
