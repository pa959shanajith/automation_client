#-------------------------------------------------------------------------------
# Name:        mobile_radio_checkbox_operations.py
# Purpose:
#
# Author:      sushma.p
#
...
from mobile_utilweb_operations import UtilWebKeywords
import mobile_table_keywords
import mobileconstants
from constants import *
import logging

log = logging.getLogger('mobile_radio_checkbox_operations.py')
...
class RadioCheckboxKeywords():
            ...
            try:
                if webelement.is_enabled():
                    ...
                    if not(visibilityFlag and is_visible):
                        mobile_browser_keywords.driver_obj.execute_script(mobileconstants.CLICK_RADIO_CHECKBOX,webelement)
                    else:
                        webelement.click()
    ...
    def __fetch_status_array(self,webelement,input):
        ...
        try:
            ...
            col_num=int(col_num)
            tag_name=input[2]
            cell=driver.execute_script(mobileconstants.GET_CELL_JS,webelement,row_num-1,col_num-1)
            element_list=cell.find_elements_by_xpath('.//*')
            if tag_name=='radio' or tag_name=='checkbox':
                log.debug('Tagname is',tag_name)
                for element in element_list:
                    element_xpath=driver.execute_script(mobileconstants.GET_XPATH_JS,element)
    ...
    def getActualElement(self,element,input):
        try:
            ...
            cell=mobile_table_keywords_obj.javascriptExecutor(element,row_number-1,col_number-1)
            element_list=cell.find_elements_by_xpath('.//*')
            for member in element_list:
                  ...
                  xpath=mobile_browser_keywords.driver_obj.execute_script(js1,member)
                  cellChild = mobile_browser_keywords.driver_obj.find_element_by_xpath(xpath)
