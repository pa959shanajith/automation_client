#-------------------------------------------------------------------------------
# Name:        mobile_textbox_operations.py
# Purpose:
#
# Author:      sushma.p
#
...
log = logging.getLogger('mobile_textbox_operations.py')
...
class TextboxKeywords:
                ...
                if webelement.is_enabled():
                    ...
                    if input is not None:
                        ...
                        if not(readonly_value is not None and readonly_value.lower() =='true' or readonly_value is ''):
                            ...
                            mobile_browser_keywords.driver_obj.execute_script(SET_TEXT_SCRIPT,webelement,input)
                    ...
                    if input is not None:
                        ...
                        if not(readonly_value is not None and readonly_value.lower() =='true' or readonly_value is ''):
                            ...
                            if not(visibilityFlag and isvisble):
                                ...
                                webelement.clear()
                                if isinstance(mobile_browser_keywords.driver_obj,mobile_browser_keywords.webdriver.Ie):
                                    log.debug('Sending the value via part 1')
                                    mobile_browser_keywords.driver_obj.execute_script(SET_TEXT_SCRIPT,webelement,input)
                                else:
    ...
    def __clear_text(self,webelement):
        ...
        mobile_browser_keywords.driver_obj.execute_script(CLEAR_TEXT_SCRIPT,webelement)
                ...
                if webelement.is_enabled():
                    ...
                    if input is not None:
                        ...
                        if not(readonly_value is not None and readonly_value.lower() =='true' or readonly_value is ''):
                            ...
                            mobile_browser_keywords.driver_obj.execute_script(SET_TEXT_SCRIPT,webelement,input_val)
                            status=TEST_RESULT_PASS
                            methodoutput=TEST_RESULT_TRUE
                        else:
                            err_msg=self.__read_only()
