...
def getOsInfo(machine_name=None):
            ...
            if machine_name is not None or platform.system().lower()=="windows":
                ...
                if wmi_ref is not None:
                    ...
                    for iterator in os_details:
                        ...
                        os_info['machine']=removingUnicodeSign(iterator.Caption)
                        os_info['version']=removingUnicodeSign(iterator.Version)
                        os_info['OSArchitecture'] = removingUnicodeSign(iterator.OSArchitecture)
                    status="Pass"
                    result = "True"
                else:
                    pass
...
var_name=[]
with open("System_getOSInfo_input.txt") as f:
    for line in f:
        if(line.strip()!=''):
            eq_index1 = line.find('#')
...
def test_myfunc(input1, expected):
    assert getOsInfo(input1) == expected
