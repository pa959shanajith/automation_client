...
def getAllProcess(machine_name=None):
            ...
            if machine_name is not None or platform.system().lower()=="windows":
                ...
                if wmi_ref is not None:
                    for process in wmi_ref.Win32_Process():
                        ...
                        value = removingUnicodeSign(process.Name)
                        value+="--"
                        value+=removingUnicodeSign(process.ProcessId)
...
var_name=[]
with open("System_getAllProcess_input.txt") as f:
    for line in f:
        if(line.strip()!=''):
            eq_index1 = line.find('#')
...
def test_myfunc(input1, expected):
    assert getAllProcess(input1) == expected
